...
salvar() {
    if (!this.pontoColeta.nome || 
        !this.pontoColeta.bairro || 
        !this.pontoColeta.endereco ||
        !this.pontoColeta.responsavel ||
        !this.pontoColeta.cpfResponsavel ||
        !this.pontoColeta.contato ||
        !this.pontoColeta.tiposResiduos || 
        this.pontoColeta.tiposResiduos.length === 0) {
      
      this.msg.add({ severity: 'warn', summary: 'Atenção', detail: 'Preencha todos os campos obrigatórios (incluindo CPF, Endereço e Responsável).' });
      return;
    }
    const cpfLimpo = this.pontoColeta.cpfResponsavel.replace(/\D/g, '');
    
    if (cpfLimpo.length !== 11) {
        this.msg.add({ severity: 'warn', summary: 'Atenção', detail: 'O CPF deve conter exatamente 11 números.' });
        return;
    }
    ...
    request$.subscribe({
      ...
      error: (err: any) => {
        const errorMsg = err.error?.message || err.error?.validationErrors?.[0]?.message || 'Erro ao salvar o ponto de coleta';
        console.error('Erro detalhado:', err);
        this.msg.add({ severity: 'error', summary: 'Erro', detail: errorMsg });
        this.loading = false;
      }
    });
  }
